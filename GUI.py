...
class ScrcpyGUI:
    ...
    def setup_ui(self):
        ...
        def on_dropdown_click(event):
            try:
                ...
                # Fallback to default behavior if method doesn't exist
                pass
    ...
    def check_existing_scrcpy_processes(self):
        ...
        if PSUTIL_AVAILABLE and psutil is not None:
            try:
                ...
                return len(existing_processes) > 0
            except Exception as e:
        ...
        try:
            if os.name == 'nt':  # Windows
                ...
                if scrcpy_lines:
                    print(f"Found {len(scrcpy_lines)} existing scrcpy process(es) via tasklist")
                return len(scrcpy_lines) > 0
            else:  # Unix-like systems
                result = subprocess.run(['pgrep', 'scrcpy'], 
                                      capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and result.stdout.strip():
                    pids = result.stdout.strip().split('\n')
                    print(f"Found {len(pids)} existing scrcpy process(es) via pgrep")
                    return True
                return False
        except Exception as e:
    ...
    def refresh_devices(self):
        ...
        def update_ui_safe(message, color="#666666"):
            """Thread-safe UI update"""
            try:
                self.device_status_label.configure(text=message, text_color=color)
        ...
        try:
            update_ui_safe("Scanning for devices...")
            ...
            if result.returncode == 0:
                ...
                if devices:
                    ...
                    update_ui_safe(f"Found {len(devices)} device(s)", "#059669")
                else:
                    self.device_combo.configure(values=[])
                    self.device_combo.set("Connected Devices:")
                    self.selected_device.set("")
                    update_ui_safe("No devices found", "#dc2626")
            else:
                update_ui_safe("ADB not found or error occurred", "#dc2626")
                self.root.after(0, lambda: messagebox.showerror("Error", "ADB not found. Please make sure ADB is installed and in PATH."))
                
        except subprocess.TimeoutExpired:
            update_ui_safe("Device scan timeout", "#dc2626")
        except FileNotFoundError:
            update_ui_safe("ADB not found", "#dc2626")
            self.root.after(0, lambda: messagebox.showerror("Error", "ADB not found. Please install Android SDK platform-tools."))
        except Exception as e:
            update_ui_safe(f"Error: {str(e)}", "#dc2626")
    ...
    def check_device_connection(self):
        ...
        # Check if process has ended (indicating potential disconnection)
        process_running = self.scrcpy_process and self.scrcpy_process.poll() is None
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if self.last_connected_device not in devices:
                    ...
                    # Only attempt reconnect if we're still supposed to be running and process isn't running
                    if self.is_running and not process_running:
                        self.attempt_reconnect()
                else:
                    # Device is back but process might have ended - restart if needed
                    if self.is_running and not process_running:
                        print(f"Device {self.last_connected_device} detected but process not running. Restarting...")
                        self.attempt_reconnect()
                    else:
                        # Reset reconnect attempts if device is back and running
                        self.reconnect_attempts = 0
        except Exception as e:
            print(f"Error checking device connection: {str(e)}")
            # If we can't check devices but process isn't running, attempt reconnect
            if self.is_running and not process_running:
                self.attempt_reconnect()
    ...
    def attempt_reconnect(self):
        ...
        if not self.auto_reconnect_enabled.get():
            print("Auto-reconnect disabled, stopping reconnection attempts.")
            return
        
        # Check if there are already scrcpy processes running
        if self.check_existing_scrcpy_processes():
            print("Existing scrcpy processes detected. Skipping reconnection attempt.")
            self.reconnect_attempts = 0
            try:
                self.reconnect_status_label.configure(text="Existing session detected, stopping auto-reconnect", text_color="#059669")
    ...
    def cleanup_process(self, timeout=3):
        """Safely cleanup scrcpy process"""
        if not self.scrcpy_process:
            return True
            
        try:
            # First try graceful termination
            if self.scrcpy_process.poll() is None:  # Process is still running
                self.scrcpy_process.terminate()
                try:
                    self.scrcpy_process.wait(timeout=timeout)
